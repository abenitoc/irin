...
void CIriFitnessFunction::SimulationStep(unsigned int n_simulation_step, double f_time, double f_step_interval)
{
	...
	/* Eval going forward */ 
	double goingForward= leftSpeed*rightSpeed;
	...
	/* Where the Max CONTACT sensor will be stored*/
	double maxGroundMemoryEval = 0.0;
	...
	/* FROM HERE YOU NEED TO CREATE YOU FITNESS */	
	 	int transitionalStates=0;
        int correctState=0;

        double fitness = maxSpeedEval * sameDirectionEval * goingForward;
	...
	fitness+=transitionalStates*0.01;
        /*
                if(redBattery[0]> 0.05 ){
                        if(groundMemory[0] == 0.0){
                                fitness *= maxRedLightSensorEval;
                        }
                }
                ...
                if(battery[0] > 0.05){
                        if(blueBattery[0] == 0.5){
                                fitness *= maxBlueLightSensorEval;
                        }
                }

                if (blueBattery[0] < 0.8){
                        if (blueBattery[0] > 0.4 ){
                                if (ground[0] == 0.0) {
	...
	m_unNumberOfSteps++;
	m_fComputedFitness += fitness + (transitionalStates);
}
